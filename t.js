        ...
        const response = await launchBrowserWithRetry(targetURL, browserProxy);
         if (response) {
            ...
            try {
                ...
                colored(nmcutii.COLOR_RED, "[INFO] Error spawning g.js: " + error.message);
            }
...
queue.drain(function() {
    colored(nmcutii.COLOR_YELLOW, "[INFO] All proxies processed — reusing proxies until time is up");

    // Nếu vẫn còn thời gian, đưa proxy vào lại queue
    const timeLeft = (duration * 1000) - (Date.now() - startTime);
    if (timeLeft > 0) {
        for (let i = 0; i < proxies.length; i++) {
            queue.push({browserProxy: proxies[i]});
        }
    }
});
    ...
    setTimeout(() => {
        ...
        exec('pkill -f g.js', (err) => {
            if (err && err.code !== 1) {
                ...
                colored(nmcutii.COLOR_GREEN, "[INFO] Successfully killed g.js processes")
            }
        });
